...
#### Filtros por patrones de texto (LIKE o NOT LIKE)

Los filtros por patrones de texto permiten buscar coincidencias parciales dentro de una cadena.
Se utilizan con los operadores:

- **LIKE** $\longrightarrow$ Busca coincidencias dentro de una cadena de texto.
- **NOT LIKE** $\longrightarrow$ Excluye las coincidencias detro de una cadena de texto. 

Son fundamentales cuando quieres seleccionar filas que coinciden con un determinado patrón, no con un valor exacto. El código que se empleará para emplear este filtro es el siguiente:

```{sql connection=con, eval=FALSE}
SELECT ...la consulta... WHERE <columna> LIKE <patrón de texto>;
```

En cuanto a la busqueda de patrones, existen dos métodos para la busqueda de los mismos dentro de cadenas de texto. Dichos métodos son:

- **Método para cualquier secuencia de caracteres (%).** En este caso los caracteres a buscar se escribirán como a, pero se pueden poner tantos como se quiera.
  - **%a** $\longrightarrow$ Termina en a. 
  - **a%** $\longrightarrow$ Empieza por a. 
  - **%a%** $\longrightarrow$ Contine la a. 
  - **%** $\longrightarrow$ Cualquier coincidencia de texto. 
- **Método para secuencas de caracteres fijos (**$\_$**).** En este caso también se empleará la a. Y cabe destacar que cada _ se corresponde con un caractér desconocido y se pueden poner tantos como se quiera.
  - $\_a$ $\longrightarrow$ Termina en a y tiene dos caracteres.
  - **a_** $\longrightarrow$ Empieza por a y tiene dos caracteres.
  - **_a_** $\longrightarrow$ Contiene la a y tiene tres caracteres.
  
A continuación hay varios ejemplos de esto, para comprobar y entender la lógica detrás de los filtros por patrones de texto. 

```{sql connection=con, tab.cap=NA, max.print = -1}
SELECT * FROM empleados WHERE nombre LIKE '%a' LIMIT 5;
```

```{sql connection=con, tab.cap=NA, max.print = -1}
SELECT * FROM empleados WHERE nombre LIKE 'C%' LIMIT 5;
```
...
```{sql connection=con, tab.cap=NA, max.print = -1}
SELECT * FROM empleados WHERE titulo LIKE '__ni__' LIMIT 5;
```

#### Comprobación de valores nulos (NULL o NOT NULL).

En una tabla cuando una columna no tiene algún dato bien en alguna fila o bien en toda la columna, en ese espacio se coloca un NULL. Para poder saber si alguna columna tiene valores nulos, se debe de realizar la siguiente query:

```{sql connection=con, eval=FALSE}
SELECT ...la consulta... WHERE <columna> IS NULL;
```

Por lo contrario si queremos extraer únicamente los datos que no son nulos, se deberá de realizar la siguiente query: 

```{sql connection=con, eval=FALSE}
SELECT ...la consulta... WHERE <columna> IS NOT NULL;
``` 

### Funciones de agregación. 

Las funciones de agregación permiten resumir o calcular valores a partir de un conjunto de filas.
En lugar de devolver filas individuales, devuelven un único resultado por grupo.

Las funciones de agregación más comunes son las siguientes:

- **COUNT(<columna> o *)** $\longrightarrow$ Permite contar filas o valores dentro de una columna. 
- **SUM(<columna>)** $\longrightarrow$ Permite sumar valores.  
- **MAX(<columna>)** $\longrightarrow$ Permite obtener el valor más elevado. 
- **MIN(<columna>)** $\longrightarrow$ Permite obtener el valor menos elevado.
- **AVG(<columna>)** $\longrightarrow$ Permite obtener la media aritmética de todos los valores. 

A continuación se puede encontrar un ejemplo con cada función de agregación.
...
**GROUP BY** sirve para agrupar filas que comparten un mismo valor en una o varias columnas.
Una vez formados los grupos, permite aplicar funciones de agregación (COUNT, SUM, AVG, etc.) para obtener un resultado por cada grupo en lugar de por cada fila.

Cuando ejecutas una consulta con GROUP BY, SQL realiza:

- Busca valores repetidos en la(s) columna(s) indicada(s).
- Agrupa todas las filas que comparten esos valores.
- Aplica una función de agregación a cada grupo.  
- Devuelve una fila por cada conjunto formado.

Para poder hacer una query básica empleando GROUP BY, se tiene que hacer del siguiente modo:

```{sql connection=con, eval=FALSE}
SELECT ...la consulta... GROUP BY <columna por la que se quiera agregar>;

-- Si se quiere, se puede agregar por más de una columna.
```

Con el fin de entender como funciona, se van a realizar algunas pruebas, empleando funciones de agregación para ver como sería también.

```{sql connection=con, tab.cap=NA, max.print = -1}
SELECT titulo FROM empleados GROUP BY titulo;
```

```{sql connection=con, tab.cap=NA, max.print = -1}
SELECT titulo, AVG(salario) AS media_salario FROM empleados GROUP BY titulo;
```

```{sql connection=con, tab.cap=NA, max.print = -1}
SELECT experiencia, max(salario) AS maximo_salario FROM empleados GROUP BY titulo;
```

### Filtros sobre grupos (HAVING).

**HAVING** es una cláusula que se utiliza para filtrar grupos completos, no filas individuales. Solo aparece en consultas que utilizan GROUP BY.

La diferencia fundamental entre HAVING y WHERE es que WHERE filtra filas individuales antes de la agrupación o agregación, mientras que HAVING filtra grupos de filas después de que se ha aplicado GROUP BY y se han calculado funciones de agregación.

Aunque el uso mas común de HAVING es junto con un GROUP BY, este se puede usar solo, tomando todas las filas como un grupo único. 

La clausula HAVING se empleará en los siguiente casos: 

- Filtrar grupos según el resultado de una agregación.
- Devolver únicamente los grupos que cumplan una condición.
- Restringir resultados después de agrupar.
- Hacer estadísticas filtradas.

La forma genérica de emplear HAVING es la siguiente: 

```{sql connection=con, eval=FALSE}
SELECT ...la consulta... HAVING <condición sobre la agregación>;
```

Para probar como funciona HAVING, se harán varias pruebas entre las cuales habrá alguna con GROUP BY para ver como interactuan.

```{sql connection=con, tab.cap=NA, max.print = -1}
SELECT COUNT(*) FROM empleados HAVING COUNT(*) > 20;
```

```{sql connection=con, tab.cap=NA, max.print = -1}
SELECT titulo, AVG(experiencia) AS media_experiencia FROM empleados
GROUP BY titulo HAVING AVG(experiencia) > 2;
```

```{sql connection=con, tab.cap=NA, max.print = -1}
SELECT experiencia, AVG(salario) AS media_salario FROM empleados
GROUP BY experiencia HAVING AVG(salario) > 25000;
```

### Subconsultas.

Una subconsulta es una consulta SQL que va dentro de otra consulta. Sirve para obtener valores, listas o tablas intermedias que la consulta principal necesita para filtrar, comparar, calcular o construir resultados más complejos. Se utiliza cuando una sola consulta no basta para resolver el problema.
