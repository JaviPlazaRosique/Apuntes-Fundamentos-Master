...
```{python eval=FALSE}
Nombre_de_la_lista = [<Elemento>, <Elemento>, <Elemento>, ...]
```
...
```{python eval=FALSE}
Nombre_de_la_lista.append(<Elemento>)
...
```{python eval=FALSE}
Nombre_de_la_lista.remove(<Elemento>)
...
```{python eval=FALSE}
Nombre_de_la_lista.pop(<Posición>)
...
```{python eval=FALSE}
Nombre_de_la_tupla = (<Elemento>, <Elemento>, <Elemento>, ...)
```
...
```{python eval=FALSE}
Nombre_de_la_tupla.count(<Elemento>)
...
```{python eval=FALSE}
Nombre_del_conjunto_mutable = [<Elemento>, <Elemento>, <Elemento>, ...]

Nombre_del_conjunto_mutable = set(Nombre_del_cojunto_mutable)

# Tambiém se puede hacer sin generar una lista anteriormente

Nombre_del_conjunto_mutable = set([<Elemento>, <Elemento>, <Elemento>, ...])
```
...
```{python eval=FALSE}
Nombre_del_conjunto_inmutable = [<Elemento>, <Elemento>, <Elemento>, ...]

Nombre_del_conjunto_inmutable = frozenset(Nombre_del_cojunto_inmutable)

# Tambiém se puede hacer sin generar una lista anteriormente

Nombre_del_conjunto_inmutable = frozenset([<Elemento>, <Elemento>, <Elemento>, ...])
```
...
```{python eval=FALSE}
Nombre_del_diccionario = {<clave>:<valor>, <clave>:<valor>, ...}
```
...
```{python eval=FALSE}
Nombre_del_diccionario.get(<clave>)
...
```{python eval=FALSE}
Nombre_del_diccionario.pop(<clave>)

# Elimina la calve y su valor asignado. También serviría:

del Nombre_del_diccionario[<clave>]
```
...
```{python eval=FALSE}
Nombre_de_la_variable_booleana = True o False
```
...
- **break** $\longrightarrow$ Sirve para detenr el bucle completamente cuando se cumple una condición, se debe de emplear if.
- **continue** $\longrightarrow$ Sirve para saltar una interacción (una vuelta del bucle) y continuar con la siguiente. 
- **else** $\longrightarrow$ Es parecido al "else" del condicional if-elif-else, pero en este caso, se ejecuta siempre que termina el bucle.
