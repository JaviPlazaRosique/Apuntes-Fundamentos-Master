...
```{python eval=FALSE}
Nombre_de_la_función = lambda <parámetros>: <expresión>
```
...
```{python eval=FALSE}
Nombre_de_la_variable_introducida = input("<mensaje que se muestra en consola>")
```
...
```{python eval=FALSE}
int(<Nombre_de_la_variable_introducida>) # Si el número es entero

float(<Nombre_de_la_variable_introducida>) # Si el número tiene decimales
```
...
La forma de definir una clase se debe de emplear el siguiente código:
